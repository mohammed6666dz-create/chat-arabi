...
async function uploadPrivateBgToServer(file) {
    ...
    try {
        ...
        if (!res.ok) throw new Error('فشل رفع الخلفية');
        const data = await res.json();
        if (data.bgUrl) return data.bgUrl;
        return null;
    } catch (err) {
...
function applyPrivateBackground(bgUrl) {
    ...
    if (bgUrl && bgUrl !== '' && bgUrl !== 'null') {
        ...
        messagesContainer.style.backgroundImage = `url("${finalUrl}")`;
        messagesContainer.style.backgroundSize = 'cover';
        messagesContainer.style.backgroundPosition = 'center';
        messagesContainer.style.backgroundRepeat = 'no-repeat';
        ...
        container.innerHTML = newsWithAvatars.map(item => `
            <div class="news-item">
                <div class="news-header" style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                    ...
                    ${myUsername === 'MOHAMED' ? `<button class="delete-news-btn" data-id="${item.id}" style="margin-right: auto; background: #ef4444; border: none; padding: 5px 10px; border-radius: 6px; color: white; cursor: pointer; font-size: 11px;">حذف</button>` : ''}
                </div>
...
async function openUserProfile(username, role = 'guest', avatar = '') {
    ...
    if (adminBox) {
        ...
        if (isOwner && !isMe) {
            ...
            `;
            setTimeout(() => {
                const showBtn = document.getElementById('showAdminCommandsBtn');
                const commandsPanel = document.getElementById('adminCommandsPanel');
                if (showBtn && commandsPanel) {
                    showBtn.onclick = (e) => {
                        e.stopPropagation();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    appBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (appName === 'wall') loadPosts();
            else if (appName === 'news') loadNews();
        });
    });
...
socket.on('friend removed', (targetName) => {
    if (window.myFriends) window.myFriends = window.myFriends.filter(f => f !== targetName);
});
...
document.getElementById('privateMsgBtn')?.addEventListener('click', () => {
    ...
    if (container) {
        container.innerHTML = '<div style="padding:20px;text-align:center;color:#aaa;">جاري تحميل المحادثات...</div>';
    }
...
socket.on('private conversations list', (conversations) => {
    ...
    if (!conversations || conversations.length === 0) {
        container.innerHTML = `<div style="padding:40px 20px; text-align:center; color:#64748b;">لا توجد محادثات سابقة</div>`;
        return;
    }
...
function showNewsSection() {
    ...
    if (addSection) {
        if (myUsername === 'MOHAMED') addSection.style.display = 'block';
...
setTimeout(() => showNewsSection(), 1000);
...
const fixAllSpaces = () => {
    ['messageInput', 'privateChatInput', 'newsTitle', 'newsContent'].forEach(id => {
        ...
        if (el) el.addEventListener('keydown', (e) => { if (e.code === 'Space') e.stopPropagation(); });
    });
};
