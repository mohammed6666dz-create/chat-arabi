...
async function createUser(username, passwordHash) {
    try {
        await pool.query(
            `INSERT INTO users (username, password_hash, rank) VALUES ($1, $2, 'ضيف')`,
...
app.post('/upload-avatar', verifyToken, upload.single('avatar'), async (req, res) => {
    ...
    if (!success) return res.status(500).json({ msg: 'خطأ في حفظ الصورة' });
...
app.post('/upload-background', verifyToken, upload.single('background'), async (req, res) => {
    ...
    if (!success) return res.status(500).json({ msg: 'خطأ في حفظ الخلفية' });
...
io.on('connection', (socket) => {
    ...
    socket.on('admin command', (data) => {
        ...
        if (username !== 'mohamed-dz') {
            return socket.emit('chat message', { 
                system: true, 
                msg: "تنبيـه: لا تملك صلاحية الإدارة." 
            });
        }
        ...
        switch (action) {
            case 'kick':
                if (targetSocket) {
                    targetSocket.emit('chat message', { 
                        system: true, 
                        msg: "لقد تم طردك من قبل الإدارة." 
                    });
                    targetSocket.disconnect();
                    io.emit('chat message', { 
                        system: true, 
                        msg: `🛑 تم طرد [${target}] بواسطة المالك.` 
                    });
                }
                break;
            ...
            case 'mute':
                if (targetSocket) {
                    targetSocket.isMuted = true;
                    targetSocket.emit('chat message', { 
                        system: true, 
                        msg: "🔇 تم كتمك من قبل الإدارة." 
                    });
                    socket.emit('chat message', { 
                        system: true, 
                        msg: `✅ تم كتم المستخدم [${target}] بنجاح.` 
                    });
                }
                break;
            ...
            case 'ban':
                ...
                io.emit('chat message', { 
                    system: true, 
                    msg: `🚫 تم حظر [${target}] نهائياً.` 
                });
                break;

            default:
                socket.emit('chat message', { system: true, msg: 'أمر إداري غير معروف' });
        }
    });
    ...
    socket.on('join', async (room, token) => {
        try {
            ...
            console.log('Error in join:', e.message);
        }
    });

    // شراء رتبة (حالياً مجاني)
    socket.on('buy role', async ({ role }) => {
        if (socket.username && role === 'premium') {
            try {
                ...
                console.error('خطأ في ترقية الرتبة:', err);
            }
        }
    });

    // إرسال رسالة عامة + فحص الكتم
    ...
    socket.on('send friend request', async (targetUsername) => {
        ...
        await pool.query(
            'UPDATE users SET friend_requests = friend_requests || $1::text, sent_requests = sent_requests || $2::text WHERE username = $3',
    ...
    // باقي أحداث الصداقة والرسائل الخاصة والـ disconnect كما هي...
