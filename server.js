...
async function broadcastOfflineUsers() {
  try {
    ...
    `);
    const offlineUsersList = allUsers.filter(user => !onlineUsernames.includes(user.username));
...
app.post('/upload-profile-song', verifyToken, upload.single('song'), async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, songUrl: songUrl, msg: 'تم رفع الأغنية بنجاح' });
  } catch (err) {
...
app.get('/api/get-friends-posts', verifyToken, async (req, res) => {
  ...
  try {
    const user = await getUser(username);
    const friendsList = user.friends || [];
    if (friendsList.length === 0) return res.json([]);
...
app.post('/api/like-post', verifyToken, async (req, res) => {
  ...
  const username = req.user.username;
  if (!postId) return res.status(400).json({ msg: 'معرف المنشور مطلوب' });
  try {
    const { rows } = await pool.query('SELECT liked_by, likes, username as post_owner FROM friends_posts WHERE id = $1', [postId]);
    if (rows.length === 0) return res.status(404).json({ msg: 'المنشور غير موجود' });
...
app.post('/api/add-comment', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (postRows.length === 0) return res.status(404).json({ msg: 'المنشور غير موجود' });
